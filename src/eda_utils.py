...
class EDAUtils:
    ...
    def calculate_loss_ratio(df):
        ...
        if "VehicleType" in valid.columns:
            vehicle = (
                valid.groupby("VehicleType")
                .apply(lambda x: x["TotalClaims"].sum() / x["TotalPremium"].sum())
                .sort_values(ascending=False)
            )

        return {"overall": overall, "by_province": province, "by_vehicle": vehicle}
    ...
    def analyze_temporal_trends(df):
        ...
        monthly["LossRatio"] = monthly.apply(
            lambda row: row["Total_Claims"] / row["Total_Premium"] if row["Total_Premium"] > 0 else 0, axis=1
        )
        monthly["MonthStr"] = monthly["YearMonth"].astype(str)

        # Get first period with claims (not first row of data)
        first_claim_row = (
            monthly[monthly["Claim_Frequency"] > 0].iloc[0]
            if len(monthly[monthly["Claim_Frequency"] > 0]) > 0
            else None
        )
    ...
    def analyze_vehicle_claims(df, top_n=10):
        ...
        vehicle_data = df[(df["make"].notna()) & (df["make"] != "") & (df["make"] != "nan") & (df["TotalPremium"] > 0)]

        if len(vehicle_data) == 0:
            return None

        # Claims by make
        make_claims = vehicle_data.groupby("make").agg({"TotalClaims": ["sum", "mean", "count"]}).round(2)
        ...
        for make in top_makes:
            models = vehicle_data[vehicle_data["make"] == make].groupby("Model").agg({"TotalClaims": "sum"}).round(2)
            models = models.sort_values("TotalClaims", ascending=False).head(5)
            top_models[make] = models

        return {"top_makes": make_claims.head(top_n), "bottom_makes": make_claims.tail(top_n), "top_models": top_models}
